...
# Extracted a number of different options into configurable settings
# Allows the user to inject a generic HTML template into each node
# The HTML template function is passed the JSON data object for each node in the tree as a parameter, and is used to inject the unique data into the template of each node.
